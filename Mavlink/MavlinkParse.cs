    ...
    public class MavlinkParse
    {
        ...
        public MAVLinkMessage ReadPacket(Stream BaseStream)
        {
            ...
            while (readcount <= MAVLink.MAVLINK_MAX_PACKET_LEN)
            {
                ...
                if (buffer[0] == MAVLink.MAVLINK_STX || buffer[0] == MAVLink.MAVLINK_STX_MAVLINK1)
                    break;
            ...
            var headerlength = buffer[0] == MAVLink.MAVLINK_STX ? MAVLink.MAVLINK_CORE_HEADER_LEN : MAVLink.MAVLINK_CORE_HEADER_MAVLINK1_LEN;
            ...
            if (buffer[0] == MAVLink.MAVLINK_STX)
            {
                ...
                if ((buffer[2] & MAVLink.MAVLINK_IFLAG_SIGNED) > 0)
                {
                    lengthtoread += MAVLink.MAVLINK_SIGNATURE_BLOCK_LEN;
                }
            }
            else
            {
            ...
            // calc crc
            ushort crc = MavlinkCRC.Calculate(buffer, buffer.Length - 2);

            // calc extra bit of crc for mavlink 1.0+
            if (message.header == MAVLink.MAVLINK_STX || message.header == MAVLink.MAVLINK_STX_MAVLINK1)
            {
                crc = MavlinkCRC.Accumulate(MAVLink.MAVLINK_MESSAGE_INFOS.GetMessageInfo(message.msgid).crc, crc);
            }
        ...
        public byte[] GenerateMAVLinkPacket10(MAVLINK_MSG_ID messageType, object indata, byte sysid = 255, byte compid = (byte)MAVLink.MAV_COMPONENT.MAV_COMP_ID_MISSIONPLANNER, int sequence = -1)
        {
            byte[] data;
            ...
            ushort checksum = MavlinkCRC.Calculate(packet, packet[1] + 6);

            checksum = MavlinkCRC.Accumulate(MAVLink.MAVLINK_MESSAGE_INFOS.GetMessageInfo((uint)messageType).crc, checksum);
        ...
        public byte[] GenerateMAVLinkPacket20(MAVLINK_MSG_ID messageType, object indata, bool sign = false, byte sysid = 255, byte compid = (byte)MAVLink.MAV_COMPONENT.MAV_COMP_ID_MISSIONPLANNER, int sequence = -1)
        {
            byte[] data;
            ...
            ushort checksum = MavlinkCRC.Calculate(packet, data.Length + MAVLink.MAVLINK_NUM_HEADER_BYTES);
